...
// NYTT SCHEMA FÖR TEAM INBJUDAN
const inviteUserSchema = z.object({
    email: z.string().email(),
    role: z.enum(['reader', 'editor', 'admin']).default('reader'),
});

// --- 6. AUTHENTICATION MIDDLEWARE (UPDATED FOR TEAMS) ---

// NYTT MIDDLEWARE: SÄKERSTÄLLER ATT ENDAST ÄGAREN ANVÄNDER DENNA ROUTE
const authorizeOwner = (req, res, next) => {
    // Kollar rollen som sattes i authenticateUser
...
                processor = proc;
                req.userRole = membership.role; // Spara rollen för framtida bruk
            }
        } else {
            req.userRole = 'owner';
        }
...
        // LOGGA EVENTET (Viktigt för Dashboard)
...
// Bjud in en ny teammedlem (Endast Ägare)
app.post('/api/team/invite', authenticateUser, authorizeOwner, async (req, res) => {
    ...
    try {
        ...
        // Kontrollera om användaren redan är aktiv medlem eller inbjuden
        ...
        if (existingInvite) return res.status(409).json({ error: 'User is already an active member or has a pending invitation.' });

        // Kontrollera om den inbjudna e-postadressen är ägarens
        if (validated.email === req.user.email) return res.status(400).json({ error: 'You are already the owner of this node.' });

        // Steg 1: Skapa inbjudan
        const inviteToken = uuidv4().replace(/-/g, '');
        const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(); // 24 timmar
...
app.get('/api/team', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        if (membersError || invitesError) throw new Error("Database fetch error.");

        // Hämta ägarens e-post
        const { data: ownerUser } = await supabase.auth.admin.getUserById(req.processor.owner_id);
        
        // Lägg till ägaren i teamlistan
        const team = [{ 
            ...
            user_id: m.user_id,
            email: m.users?.email || 'Unknown User', 
            role: m.role, 
            status: 'Active' 
        })));
        ...
        res.json({ team: team, pending: pending, userRole: req.userRole });
...
// Rutt för att ta bort en teammedlem (endast ägare)
app.delete('/api/team/member/:userId', authenticateUser, authorizeOwner, async (req, res) => {
    const { userId } = req.params;
    
    // Förhindra att ägaren tar bort sig själv
    if (userId === req.processor.owner_id) return res.status(400).json({ error: 'Cannot remove the owner of the processor node.' });

    try {
        // Steg 1: Ta bort medlemmen från processor_users
        ...
        // Steg 2: LOGGA EVENTET OCH UPPDATERA MERKLE TREE (Fullständig logik)
...
// PROCESSOR CREATION (MED DOMÄN-CHECK)
app.post('/api/processors', authenticateUser, async (req, res) => {
    const { companyName, plan } = req.body;
    
    // 1. Check om användaren redan har en processor (som ägare eller medlem)
...
    // 3. Check om domänen är upptagen
