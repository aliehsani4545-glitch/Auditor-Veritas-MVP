...
export const apiCall = async (endpoint, options = {}, token = null, apiKey = null) => {
  ...
  // Här fångar vi felet så att catch-blocket nedan kan hantera 404
...
function App() {
  ...
  const [processor, setProcessor] = useState(null); // null = loading, false = no processor (show create), object = has processor
  ...
  // --- UPDATED FETCH DASHBOARD FUNCTION ---
  const fetchDashboard = useCallback(async () => { 
    if (!session?.access_token) return; 
    // Vi behåller processor som null (loading) eller det gamla värdet tills vi vet.
    // Men om vi vill visa spinner direkt kan vi sätta null:
    setProcessor(null); 
    ...
    try { 
      ...
      // HÄR SKER MAGIN: Hantera 404 för nya användare
      if(error.message.includes('404') || error.message.includes('Processor not found') || error.message.includes('Not Found')) {
        console.log("New user detected (404 on dashboard). Redirecting to creation flow.");
        setProcessor(false); // Detta triggar <CreateProcessor /> i JSX
      } else {
  ...
  // Om användaren inte accepterat villkoren.
  // Notera: Lade till onClose för att hantera "Decline"-klicket (skickar användaren bort från sidan).
  if (!privacyAccepted) return (
    <PrivacyPage 
        onAccept={() => { localStorage.setItem('av_privacy_v1', 'true'); setPrivacyAccepted(true); }} 
        onClose={() => { alert("You must accept the terms to use this service."); window.location.href = "https://google.com"; }} 
    />
  );
