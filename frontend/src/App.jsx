...
// ... (KeyRotationComponent is same as before, kept short here for brevity) ...
const KeyRotationComponent = ({ processor, currentKey, onKeyUpdate }) => {
  const [showKey, setShowKey] = useState(false);
  // (Samma logik som förut)
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex justify-between mb-4"><h3 className="font-bold flex items-center gap-2 text-slate-800"><RotateCw className="w-5 h-5 text-purple-600"/> Key Rotation</h3><span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded font-bold">Active</span></div>
        <div className="relative mb-4"><div className="bg-slate-50 p-3 rounded-lg text-xs font-mono text-slate-600 break-all border border-slate-100 pr-20 min-h-[40px] flex items-center">{showKey ? currentKey : '••••••••••••••••••••••••••••••'}</div><div className="absolute right-2 top-1/2 -translate-x-1/2 flex gap-1"><button onClick={() => setShowKey(!showKey)} className="p-1.5 hover:bg-slate-200 rounded text-slate-500"><Eye size={14}/></button><button onClick={() => {navigator.clipboard.writeText(currentKey); alert("Copied!");}} className="p-1.5 hover:bg-slate-200 rounded text-slate-500"><Copy size={14}/></button></div></div>
        <button onClick={() => onKeyUpdate(`av_new_${Math.random().toString(36).substr(2,8)}`)} className="w-full bg-slate-900 text-white py-3 rounded-xl text-sm font-bold hover:bg-slate-800">Rotate Key Now</button>
    </div>
  );
};
...
function App() {
  ...
  useEffect(() => {
    const savedPrivacy = localStorage.getItem('privacyAccepted_v8');
  ...
  const handlePrivacyAccept = () => {
    localStorage.setItem('privacyAccepted_v8', 'true');
    setPrivacyAccepted(true);
  };

  // Reset Privacy for Footer Link
  const openPrivacy = () => {
  ...
  const handleLogEvent = async (e) => {
    ...
    if (stats.totalEvents >= 100) {
        alert("⚠️ Usage Limit Reached (100 Events).\n\nPlease upgrade to Professional Plan to continue logging events.");
        return;
    }
      ...
      <header className="fixed w-full top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 transition-all">
        ...
        {mobileMenuOpen && (
          <div className="absolute top-20 left-0 w-full bg-white border-b border-slate-200 p-6 flex flex-col gap-4 md:hidden shadow-xl">
             <button onClick={() => setActiveTab('home')} className="text-left font-bold text-slate-700">Product</button>
             <button onClick={() => setActiveTab('pricing')} className="text-left font-bold text-slate-700">Pricing</button>
             <button onClick={() => setActiveTab('dashboard')} className="w-full bg-[#0a2540] text-white py-3 rounded-xl">Sign In</button>
          </div>
        )}
      </header>
    );
           ...
           <div className="pt-32 px-6 max-w-7xl mx-auto pb-20">
             ...
             <div className="grid md:grid-cols-3 gap-8">
               ...
               <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-lg flex flex-col">
                 ...
                 <p className="text-4xl font-black mb-6">$0<span className="text-base text-slate-400 font-normal">/mo</span></p>
                 <ul className="space-y-3 mb-8 flex-1">
                   {['100 Events / Month', 'Audit Logs', 'GDPR Compliance'].map(f=><li key={f} className="flex gap-2 text-sm text-slate-600"><Check className="w-4 h-4 text-green-500"/>{f}</li>)}
                 </ul>
               ...
               <div className="bg-white p-8 rounded-3xl border-2 border-[#635bff] shadow-2xl transform scale-105 flex flex-col relative">
                 ...
                 <p className="text-4xl font-black mb-6">$49<span className="text-base text-slate-400 font-normal">/mo</span></p>
                 <ul className="space-y-3 mb-8 flex-1">
                   {['50,000 Events / Month', 'Advanced Analytics', 'Priority Support', 'Key Rotation API'].map(f=><li key={f} className="flex gap-2 text-sm text-slate-600"><Check className="w-4 h-4 text-[#635bff]"/>{f}</li>)}
                 </ul>
                 <button className="w-full py-3 rounded-xl font-bold bg-[#635bff] text-white hover:bg-[#5449e3] cursor-not-allowed opacity-80" title="Coming Soon">Select Plan</button>
               </div>
               {/* Enterprise */}
               <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-lg flex flex-col">
                 <h3 className="text-2xl font-bold mb-2">Enterprise</h3>
                 <p className="text-4xl font-black mb-6">$199<span className="text-base text-slate-400 font-normal">/mo</span></p>
                 <ul className="space-y-3 mb-8 flex-1">
                   {['Custom Events Limit', 'Dedicated Support', 'SLA Guarantee', 'Custom Integrations'].map(f=><li key={f} className="flex gap-2 text-sm text-slate-600"><Check className="w-4 h-4 text-green-500"/>{f}</li>)}
                 </ul>
                 <button className="w-full py-3 rounded-xl font-bold bg-slate-900 text-white hover:bg-slate-800 cursor-not-allowed opacity-80" title="Contact Sales">Contact Sales</button>
               </div>
             </div>
           </div>
        )}
             ...
             {!processor ? (
               ...
               <div className="max-w-6xl mx-auto animate-fade-in-up">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><h3 className="font-bold text-slate-700">Usage</h3><p className="text-3xl font-bold text-slate-900">{stats.monthlyEvents} / {stats.totalEvents >= 100 ? '100 (Max)' : '100'}</p></div>
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-slate-100">
                      ...
                      <form onSubmit={handleLogEvent} className="space-y-4">
                        <input type="text" placeholder="Event Type" className="w-full p-3 border rounded-xl" value={eventData.event_type} onChange={e => setEventData({...eventData, event_type: e.target.value})} required />
                        <textarea placeholder='JSON Payload' className="w-full p-3 border rounded-xl font-mono text-sm" rows="3" value={eventData.event_data} onChange={e => setEventData({...eventData, event_data: e.target.value})} required />
                        <input type="text" placeholder="User ID" className="w-full p-3 border rounded-xl" value={eventData.user_identifier} onChange={e => setEventData({...eventData, user_identifier: e.target.value})} />
