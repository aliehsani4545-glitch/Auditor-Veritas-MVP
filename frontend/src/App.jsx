      ...
      <div className="container mx-auto px-4 sm:px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer hover:opacity-90 transition" onClick={() => { setActiveTab('pricing'); setMobileMenuOpen(false); }}>
            <div className="bg-white/10 p-2 rounded-lg"><ShieldCheck className="w-6 h-6 text-emerald-400" /></div>
            <div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
    <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200 hover:shadow-md transition">
      <div className="flex items-center mb-4">
        ...
        <h3 className="ml-3 text-sm font-semibold text-slate-600 uppercase">Monthly Usage</h3>
      </div>
      <div className="text-3xl font-bold text-slate-900 mb-2">{stats.monthlyEvents}</div>
      <div className="w-full bg-slate-100 rounded-full h-2">
        <div className="bg-blue-600 h-2 rounded-full" style={{ width: stats.utilization }}></div>
      </div>
    </div>
    <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200 hover:shadow-md transition">
      <div className="flex items-center mb-4">
        <div className="p-2 bg-amber-50 rounded-lg"><Zap className="w-6 h-6 text-amber-500" /></div>
        <h3 className="ml-3 text-sm font-semibold text-slate-600 uppercase">Plan Status</h3>
      </div>
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200 hover:shadow-md transition">
      <div className="flex items-center mb-4">
        ...
        <h3 className="ml-3 text-sm font-semibold text-slate-600 uppercase">Total Events</h3>
      </div>
...
const HowItWorks = ({ setActiveTab, cookiesAccepted, onAcceptCookies }) => {
  ...
  const steps = [
    { 
      ...
      details: ["256-bit API key generation", "Instant secure container provisioning"], 
      visual: "key", 
      action: "create", 
    ...
    { 
      ...
      details: ["Simple REST API integration", "Automatic SHA-256 hashing"], 
      visual: "code", 
      action: "events", 
    ...
    { 
      ...
      details: ["Cryptographic chain linking", "Tamper-evident design"], 
      visual: "chain", 
      action: "privacy", 
    ...
    { 
      ...
      details: ["Live activity monitoring", "Compliance reporting"], 
      visual: "dashboard", 
      action: "dashboard", 
  ...
  const simulateTamper = () => {
    ...
    setChain(newChain);
    setTimeout(() => { 
      setChain(newChain.map(block => ({ ...block, status: 'invalid', hash: 'HACKED_' + Math.random().toString(36).substring(2, 8) }))); 
    }, 600);
    setTimeout(() => { 
      setChain([
        { id: 1, hash: 'a1b2c3d4', prevHash: '0000', status: 'valid', timestamp: Date.now() - 3000 }, 
          ...
          <div className="bg-slate-900 rounded-3xl border border-slate-700 font-mono text-sm overflow-hidden h-96 flex flex-col shadow-2xl">
            ...
            <div className="p-6 text-slate-300 flex-grow overflow-y-auto space-y-4 bg-gradient-to-br from-slate-900 to-slate-800">
              ...
              {!isSimulating && !terminalOutput && (
                <button onClick={simulateApiCall} className="mt-6 bg-gradient-to-r from-emerald-600 to-cyan-600 text-white px-6 py-3 rounded-xl text-sm font-medium hover:from-emerald-700 hover:to-cyan-700 transition-all duration-300 flex items-center shadow-lg hover:shadow-xl transform hover:scale-105">
          ...
          <div className={`bg-gradient-to-br ${currentStep.bgGradient} rounded-3xl p-8 border border-slate-200 h-96 flex flex-col relative overflow-hidden`}>
            ...
            <div className="flex-1 flex flex-col justify-center items-center relative z-10">
              <div className="flex justify-center space-x-6 mb-8">
                {chain.map((block, i) => (
                  <div key={block.id} className="flex flex-col items-center">
                    <div className={`w-20 h-20 rounded-2xl border-3 flex flex-col items-center justify-center transition-all duration-500 transform ${
                      block.status === 'valid' ? 'bg-white border-emerald-400 shadow-lg hover:scale-110' : 'bg-red-50 border-red-500 shadow-2xl scale-110'
                    } ${isTampering ? 'animate-pulse' : ''} cursor-pointer hover:shadow-xl`}>
                      {block.status === 'valid' ? <Check className="w-8 h-8 text-emerald-500"/> : <AlertTriangle className="w-8 h-8 text-red-500"/>}
                      <span className="text-xs mt-2 font-mono font-bold">#{block.id}</span>
                    </div>
                    {i < chain.length - 1 && (
                      <div className={`w-12 h-1 mx-1 transition-all duration-500 ${
                        block.status === 'invalid' && chain[i + 1]?.status === 'invalid' ? 'bg-red-300' : 'bg-slate-300'
                      }`}></div>
                    )}
                  </div>
                ))}
              ...
              <div className={`mb-6 p-5 rounded-2xl transition-all duration-500 w-full max-w-md text-center backdrop-blur-sm ${
                isTampering ? 'bg-red-100/80 border border-red-200 text-red-800' : 'bg-emerald-100/80 border border-emerald-200 text-emerald-800'
              }`}>
                ...
                <div className="mt-3 flex justify-between text-sm">
                  <span className="font-bold">Score: {securityScore}%</span>
            ...
            <div className="flex justify-center space-x-6 items-end relative z-10">
              <button onClick={simulateTamper} disabled={isTampering} className={`text-sm border-2 px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 ${
                isTampering ? 'opacity-50 cursor-not-allowed' : 'border-red-300 text-red-600 hover:bg-red-50 hover:border-red-400 hover:shadow-lg'
              }`}>
                🚨 Simulate Attack
              </button>
              <div className="flex-1 max-w-md">
                <div className="flex space-x-3">
                  ...
                  <button onClick={calculateHash} disabled={isHashing} className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-6 py-3 rounded-xl text-sm font-semibold hover:from-blue-700 hover:to-cyan-700 transition-all duration-300 disabled:opacity-50 transform hover:scale-105 shadow-lg">
                    {isHashing ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'Hash Data'}
                  </button>
                </div>
                {hashOutput && (
        ...
        return (
          <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-3xl p-8 border border-slate-200 h-96 flex flex-col relative overflow-hidden">
                      ...
                      <div key={event.id} className="p-4 hover:bg-white/50 transition-all duration-300 group">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-3">
                            ...
                            <span className="font-semibold text-sm text-slate-800 group-hover:text-blue-600 transition-colors">{event.type}</span>
                          </div>
                          <span className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full">{event.timestamp}</span>
                        </div>
            ...
            <div className="flex justify-between mt-6 relative z-10">
              <button onClick={handleExportLogs} className="text-sm bg-blue-600 text-white px-4 py-2 rounded-xl hover:bg-blue-700 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center">
                <Download className="w-4 h-4 mr-2" />Export
              </button>
              <button onClick={handleGenerateReport} className="text-sm bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center">
                <FileText className="w-4 h-4 mr-2" />Report
              </button>
              <button onClick={handleViewAnalytics} className="text-sm bg-purple-600 text-white px-4 py-2 rounded-xl hover:bg-purple-700 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
          <div className="hidden lg:grid grid-cols-4 bg-slate-50/50 border-b border-slate-200/50">
            {steps.map((step, index) => (
              <button key={index} onClick={() => setActiveStep(index)} className={`p-6 text-sm font-semibold transition-all duration-500 flex flex-col items-center justify-center group relative overflow-hidden ${
                activeStep === index ? `bg-gradient-to-r ${step.gradient} text-white shadow-lg` : 'text-slate-600 hover:text-slate-800 hover:bg-white/50'
              }`}>
            ...
            <div className="w-full lg:w-2/5">
              <div className="lg:sticky lg:top-12 space-y-6">
                ...
                <div className="flex space-x-4 pt-4">
                  <button onClick={() => cookiesAccepted && setActiveTab(currentStep.action)} disabled={!cookiesAccepted} className={`px-6 py-3 lg:px-8 lg:py-4 rounded-xl font-bold transition-all duration-300 flex items-center space-x-3 ${
                    cookiesAccepted ? `bg-gradient-to-r ${currentStep.gradient} text-white hover:shadow-xl transform hover:scale-105 shadow-lg` : 'bg-slate-300 text-slate-500 cursor-not-allowed'
                  }`}>
                    <span className="text-sm lg:text-base">{currentStep.actionText}</span>
                    <ArrowRight className="w-4 h-4 lg:w-5 lg:h-5" />
                  </button>
                  {activeStep > 0 && (
                    <button onClick={() => setActiveStep(activeStep - 1)} className="px-6 py-3 lg:px-8 lg:py-4 rounded-xl font-bold text-slate-600 hover:text-slate-800 hover:bg-slate-100 transition-all duration-300 border border-slate-200 transform hover:scale-105">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 px-4 sm:px-0">
              {Object.entries(pricingPlans).map(([key, plan]) => (
                <div key={key} className={`relative rounded-2xl p-6 sm:p-8 transition-all duration-300 hover:scale-105 hover:shadow-xl flex flex-col ${
                  ...
                  <button onClick={() => setActiveTab('create')} className="w-full py-3 sm:py-3.5 rounded-xl font-bold bg-slate-100 text-slate-800 hover:bg-slate-200 transition">
          ...
          <div className="animate-in px-4 sm:px-0">
            {!cookiesAccepted ? (
              ...
              <div className="max-w-md mx-auto bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-100 mt-6 sm:mt-10">
                ...
                <button onClick={fetchDashboard} disabled={isLoading} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 transition">
              ...
              <div>
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900">{processor.companyName}</h2>
                    <p className="text-sm text-slate-500">Dashboard</p>
                  </div>
                  <div className="flex space-x-2">
                    <button onClick={handleGDPRExport} className="px-4 py-2 bg-slate-50 border rounded-lg text-sm font-medium hover:bg-slate-100 flex items-center">
                      <Download className="w-4 h-4 mr-2 text-blue-600"/> Export Data
                    </button>
                    <button onClick={handleKeyRotation} className="px-4 py-2 bg-amber-50 border border-amber-200 rounded-lg hover:bg-amber-100 hover:shadow-sm transition text-amber-700 font-medium text-sm flex items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white p-6 rounded-xl border border-slate-100">
                    <div className="flex justify-between mb-4">
                      ...
                      {processor.plan !== 'starter' && <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded">Active</span>}
                    </div>
                    {processor.plan === 'starter' ? 
                      <LockedFeature title="Analytics Locked" desc="Upgrade to view insights." setActiveTab={setActiveTab} /> : 
                      <div className="h-48 bg-blue-50 rounded-xl flex items-center justify-center text-blue-400">
                  ...
                  <div className="bg-white p-6 rounded-xl border border-slate-100">
                    ...
                    {processor.plan === 'starter' ? 
                      <LockedFeature title="Bulk Import Locked" desc="Process CSV files." setActiveTab={setActiveTab} /> : 
