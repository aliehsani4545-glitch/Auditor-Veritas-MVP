...
const HeroTypewriter = ({ text, delay = 50 }) => {
  ...
  return (
    <p className="text-lg text-slate-300 max-w-xl min-h-[3.5rem] leading-relaxed">
      {currentText}
...
function App() {
  ...
  const handlePrivacyAccept = () => {
    localStorage.setItem('privacyAccepted_v11', 'true');
  ...
  const handleLogEvent = async (e) => {
    ...
    if (stats.totalEvents >= 100) {
      alert("⚠️ Usage Limit Reached (100 Events).\nPlease contact Sales for Enterprise Access.");
      return;
    }
    setIsLoading(true);
    try {
      ...
      <header className="fixed w-full top-0 z-50 bg-white/5 backdrop-blur-md border-b border-white/10 transition-all">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('home')}>
            ...
            <span className="font-bold adaptive-text text-xl">Auditor Veritas</span>
          </div>
          <nav className="hidden md:flex gap-1 items-center p-1 rounded-xl">
            {['home', 'create', 'pricing', 'dashboard'].map((tab) => (
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors capitalize ${activeTab === tab ? 'bg-white text-slate-900 shadow-sm' : 'adaptive-text hover:bg-white/10'}`}
              >
        ...
        {mobileMenuOpen && (
          <div className="absolute top-20 left-0 w-full bg-slate-900 border-b border-slate-800 p-6 flex flex-col gap-4 md:hidden shadow-xl">
             {['home', 'create', 'pricing', 'dashboard'].map((tab) => (
               <button key={tab} onClick={() => {setActiveTab(tab); setMobileMenuOpen(false)}} className="text-left font-bold text-white capitalize py-2">
                 {tab === 'pricing' ? 'Enterprise' : tab}
               </button>
             ))}
          </div>
      ...
      <main className="flex-1 pt-0">
        {activeTab === 'home' && (
          <>
            ...
            {/* CODE INTEGRATION (Light) */}
            <div id="code-integration" className="bg-white">
                <CodeIntegration />
            </div>

            {/* USE CASES (Dark) */}
            <div id="use-cases" className="bg-[#0a2540]">
                <UseCases />
            </div>
            
            {/* INTEGRITY ENGINE (Light) */}
            <div id="merkle" className="bg-slate-50 py-10">
                <IntegrityEngine />
            </div>
            
            {/* ARCHITECTURE (Dark) */}
            <div id="architecture" className="bg-[#0f172a]">
               <CoreArchitecture /> 
            </div>
          </>
        )}
               ...
               <div className="max-w-6xl mx-auto animate-fade-in-up">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-slate-100">
                      <h3 className="text-xl font-bold mb-6 flex items-center gap-2"><Zap className="text-amber-500"/> Log New Event</h3>
                      <form onSubmit={handleLogEvent} className="space-y-4">
                        <div><label className="block text-sm font-bold text-slate-700 mb-1">Event Type</label><input type="text" placeholder="e.g. login" className="w-full p-3 border rounded-xl" value={eventData.event_type} onChange={e => setEventData({...eventData, event_type: e.target.value})} required /></div>
                        <div><label className="block text-sm font-bold text-slate-700 mb-1">JSON Payload</label><textarea placeholder='{"ip": "1.1.1.1"}' className="w-full p-3 border rounded-xl font-mono text-sm h-32" value={eventData.event_data} onChange={e => setEventData({...eventData, event_data: e.target.value})} required /></div>
                        <div><label className="block text-sm font-bold text-slate-700 mb-1">User ID (Auto-Hashed)</label><input type="text" placeholder="user@example.com" className="w-full p-3 border rounded-xl" value={eventData.user_identifier} onChange={e => setEventData({...eventData, user_identifier: e.target.value})} /></div>
        ...
        {activeTab === 'create' && (
            <div className="pt-32 p-6 flex justify-center bg-slate-50 min-h-screen text-slate-900">
                <CreateProcessor />
            </div>
        )}
      </main>
