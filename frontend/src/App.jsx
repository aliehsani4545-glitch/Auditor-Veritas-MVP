...
import { 
  ...
  GitBranch, Network, ChevronRight, Copy, Eye, EyeOff
} from 'lucide-react';
...
// Interactive Merkle Demo Component
    ...
    <div className="py-24 bg-slate-50 border-t border-slate-200">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-500 mt-2">Trace how data becomes immutable.</p>
        </div>
        <div className="flex flex-col lg:flex-row gap-12 items-center justify-center">
          <div className="w-full lg:w-1/3 space-y-4">
             {['Raw Data Capture', 'SHA-256 Hashing', 'Merkle Tree Leaf', 'Root Hash Update'].map((label, i) => (
               <div key={i} className={`p-4 rounded-xl border transition-all ${step >= i ? 'bg-blue-50 border-blue-200 shadow-sm' : 'bg-white'}`}>
                 <h4 className={`font-bold ${step >= i ? 'text-blue-700' : 'text-slate-400'}`}>{i+1}. {label}</h4>
               </div>
             ))}
          ...
          <div className="w-full lg:w-1/2 bg-[#0f172a] p-8 rounded-3xl shadow-2xl min-h-[300px] flex flex-col items-center justify-center relative overflow-hidden">
             <div className="absolute inset-0 bg-gradient-to-br from-slate-800 to-slate-950 opacity-50"></div>
             <motion.div animate={{ scale: step === 3 ? 1.2 : 1, borderColor: step === 3 ? '#10b981' : '#475569' }} className="w-20 h-20 bg-slate-800 rounded-full flex items-center justify-center text-white font-bold z-10 shadow-lg border-2 border-slate-600 mb-8">ROOT</motion.div>
             <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
                <path d="M 50 35 L 30 70" stroke={step >= 3 ? "#10b981" : "#334155"} strokeWidth="0.5" />
                <path d="M 50 35 L 70 70" stroke={step >= 3 ? "#10b981" : "#334155"} strokeWidth="0.5" />
             </svg>
             <div className="flex gap-8 z-10">
                <motion.div animate={{ y: step >= 1 ? 0 : 50, opacity: step >= 1 ? 1 : 0 }} className="w-16 h-16 bg-blue-600/20 border border-blue-500 rounded-xl flex items-center justify-center text-[10px] text-blue-400 font-mono">{step >= 2 ? '0x9A...' : 'Data'}</motion.div>
                <div className="w-16 h-16 bg-slate-800/50 border border-slate-700 rounded-xl"></div>
             </div>
          </div>
        </div>
      </div>
...
function App() {
  ...
  useEffect(() => {
    const savedPrivacy = localStorage.getItem('privacyAccepted_final_v4');
  ...
  const handlePrivacyAccept = () => {
    localStorage.setItem('privacyAccepted_final_v4', 'true');
  ...
  if (!privacyAccepted) return <PrivacyGate onAccept={handlePrivacyAccept} />;
            ...
            <div className="relative bg-[#0a2540] text-white min-h-[90vh] flex items-center overflow-hidden">
              ...
              <div className="relative z-10 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                <div className="space-y-8">
                  ...
                  <button onClick={() => setActiveTab('create')} className="px-8 py-4 rounded-full bg-[#635bff] hover:bg-[#5449e3] font-bold shadow-lg transition-transform">Start Integration</button>
                </div>
