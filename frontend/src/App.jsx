    ...
    <>
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
            {!showConfirm ? (
                <button onClick={() => setShowConfirm(true)} className="bg-slate-900 text-white py-2.5 rounded-xl text-xs font-bold hover:bg-slate-800 transition-all">Rotate Key</button>
            ) : (
                <div className="flex gap-2">
                    <button onClick={rotate} disabled={isRotating} className="flex-1 bg-red-600 text-white py-2.5 rounded-xl text-xs font-bold hover:bg-red-700 flex justify-center items-center">{isRotating ? <RefreshCw className="animate-spin w-3 h-3"/> : 'Confirm'}</button>
      ...
      {newKeyData && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-slate-200">
                <div className="text-center mb-6"><div className="w-12 h-12 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-3"><RotateCw size={24} /></div><h3 className="text-xl font-bold text-slate-900">New Key Generated</h3><p className="text-sm text-slate-500 mt-1">This is the only time we will show this key.</p></div>
                ...
                <button onClick={() => { setNewKeyData(null); setKeyToDisplay(newKeyData); }} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/20">I have saved this key safely</button>
            </div>
        </div>
      )}
    </>
