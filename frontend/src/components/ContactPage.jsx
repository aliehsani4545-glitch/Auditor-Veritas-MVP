...
import { Mail, MapPin, ShieldCheck, Send, CheckCircle, Loader2, Zap, Lock, Cookie, AlertCircle } from 'lucide-react';

// Reusing the InputGroup from EnterpriseForm (assuming it's available globally or imported)
const InputGroup = ({ label, name, required, disabled, ...props }) => (
    <div className="space-y-1">
      ...
      <input name={name} {...props} className="w-full bg-slate-950/50 border border-slate-700/50 rounded-xl p-4 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all disabled:opacity-50" disabled={disabled} />
    </div>
);
...
// --- Cookie Consent Component för ContactPage ---
const ContactCookieConsent = ({ onAccept, onDecline, onReadPolicy }) => {
    return (
        <motion.div 
            ...
            className="absolute inset-0 flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-6 rounded-[2.5rem] z-30" 
        >
            <div className="bg-slate-800 p-6 rounded-xl shadow-2xl border border-slate-700 max-w-sm text-center text-white">
                <Cookie size={32} className="text-blue-400 mx-auto mb-3" />
                <h4 className="text-xl font-bold mb-2">Data Processing Consent Required</h4>
                <p className="text-sm text-slate-400 mb-5">
                    To submit this form, we need your consent to process the personal data you enter, as outlined in our privacy policy.
                </p>
                <div className="flex flex-col gap-3">
                    ...
                    <button 
                        onClick={onReadPolicy} // Leder till PrivacyPage
                        className="bg-slate-700 text-white px-4 py-3 rounded-lg font-bold text-sm hover:bg-slate-600 transition-colors"
                    >
                        Read Full Policy
                    </button>
                </div>
                <button onClick={() => onDecline('denied')} className="text-xs text-slate-500 mt-4 hover:text-slate-300 underline">Decline</button>
            </div>
        </motion.div>
    );
};
...
const ContactPage = ({ unifiedCookieStatus, onUnifiedConsent, onOpenPrivacy }) => { // Nyckelfunktion: Tar emot props
  const [formState, setFormState] = useState({ name: '', email: '', message: '' });
  const [status, setStatus] = useState('idle'); // idle, submitting, success, error
  
  // ANVÄNDER GLOBAL STATUS istället för lokal contact_cookie_consent
...
    e.preventDefault();
    if (formInteractionBlocked || unifiedCookieStatus === 'denied') {
        setStatus('error');
        return;
    }
...
                        
                        {status === 'error' && unifiedCookieStatus === 'denied' && (
                            <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="bg-red-900/50 p-3 rounded-lg border border-red-700/50 text-red-400 text-sm flex items-center gap-2">
                                <AlertCircle size={16}/> Submission failed: Consent to data processing is required.
                            </motion.div>
                        )}
...
                                required 
                                className="w-full bg-slate-950/50 border border-slate-700/50 rounded-xl p-4 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all disabled:opacity-50"
                                placeholder="John Doe"
                                disabled={formInteractionBlocked || unifiedCookieStatus === 'denied'}
                            />
                        </div>
...
                                required 
                                rows="4"
                                className="w-full bg-slate-950/50 border border-slate-700/50 rounded-xl p-4 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all resize-none disabled:opacity-50"
...
                        <button 
                            type="submit" 
                            disabled={status === 'submitting' || formInteractionBlocked || unifiedCookieStatus === 'denied'}
                            className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-blue-900/20 flex items-center justify-center gap-3 group relative overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
                        >
