...
// Helper for API calls (assuming same setup as Dashboard)
...
const MerkleProofViewer = ({ apiKey }) => {
  ...
  const handleVerify = async (e) => {
    e.preventDefault();
    if (!eventId || !apiKey) {
      setError('Please provide both an Event ID and a valid API Key.');
      return;
    }
    ...
    try {
      ...
      setError(err.message.includes('404') ? 'Event not found or does not belong to your Processor.' : `Verification Failed: ${err.message}`);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className={`p-5 flex justify-between items-center ${proofResult.isValid ? 'bg-emerald-50' : 'bg-red-50'}`}>
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-lg text-slate-800">
                {proofResult.isValid ? 'Verification SUCCESS' : 'INTEGRITY WARNING'}
              </span>
            </div>
