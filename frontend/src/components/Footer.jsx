...
// --- SUB-KOMPONENTER (UPPDATERAD MED PLACERINGS-URL) ---
const SocialLink = ({ icon: Icon, href }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-all">
    <Icon size={18} />
  </a>
);
...
  // FIXAD LOGIK: Navigerar till Home FÖRST, sedan scrollar.
  const handleScrollTo = (id) => {
    // Byt alltid till Home för att säkerställa att mål-IDt existerar i DOM
    onNavigate('home'); 
    
    setTimeout(() => {
...
      }
    }, 50); // Liten fördröjning för att ge React tid att rendera Home-sidan
  };
...
            body: encode({ "form-name": "contact_footer", ...formData }) 
        });
...
               {/* PLATSHÅLLARE: Byt till riktiga länkar vid lansering */}
               <SocialLink icon={Github} href="https://github.com/AuditorVeritas-MVP" />
               <SocialLink icon={Twitter} href="https://twitter.com/AuditorVeritas" />
               <SocialLink icon={Linkedin} href="https://linkedin.com/company/AuditorVeritas" />
            </div>
          </div>
          
          {/* PLATFORM LINKS */}
...
               </li>
               <li>
                 {/* FIX: API Reference by scrolling to the integration section on the Home page */}
                 <button 
                    onClick={() => handleScrollTo('code-integration')}
...
               </li>
               <li>
                 {/* UX FIX: System Status should not use a simple JS alert for professionalism */}
                 <button onClick={() => window.open('https://auditorveritas-status.com', '_blank')} className="hover:text-emerald-400 transition-colors flex items-center gap-2">
...
               </li>
               <li>
                  {/* FIX: Navigerar till Trust Center */}
                  <button onClick={() => onNavigate('trust')} className="hover:text-blue-400 transition-colors flex items-center gap-2">
                    <ShieldCheck size={14}/> Security Architecture
                  </button>
               </li>
               <li>
                  {/* FIX: Navigerar till Trust Center */}
                  <button onClick={() => onNavigate('trust')} className="hover:text-white transition-colors">GDPR & Logs</button>
               </li>
            </ul>
          </div>
        </div>
