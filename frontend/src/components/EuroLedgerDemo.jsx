...
const FloatingNode = ({ icon: Icon, title, subtitle, align = "left", active }) => (
    <motion.div
        ...
        className={`absolute top-1/2 -translate-y-1/2 ${align === "left" ? "left-4 md:-left-20" : "right-4 md:-right-20"} 
          hidden md:flex flex-col items-center justify-center p-5 rounded-2xl 
          backdrop-blur-xl border shadow-xl z-10 w-40 h-44
          ${active 
            ? 'bg-white border-blue-200 shadow-blue-900/10' 
            : 'bg-white/50 border-slate-200 shadow-slate-200/50'} transition-all duration-700`}
    >
        <div className={`relative w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-all duration-500 ${active ? 'bg-blue-50' : 'bg-slate-100'}`}>
            <Icon size={28} className={active ? "text-blue-600" : "text-slate-500"} />
            {active && (
                <>
                    <motion.div 
        ...
        <div className="text-center">
            {/* ÖKAD KONTRAST HÄR */}
            <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${active ? "text-slate-900" : "text-slate-600"}`}>{title}</div>
            <div className={`text-[11px] font-mono font-medium ${active ? "text-blue-600" : "text-slate-500"}`}>{subtitle}</div>
        </div>
    </motion.div>
);
    ...
    <div className={`absolute top-1/2 -translate-y-1/2 ${align === "left" ? "left-[90px] w-[200px]" : "right-[90px] w-[200px]"} h-[2px] hidden md:block -z-10 overflow-visible`}>
        ...
        <AnimatePresence>
            {active && (
                <>
                    <motion.div 
                        className="absolute top-1/2 -translate-y-1/2 h-[3px] bg-gradient-to-r from-transparent via-blue-500 to-transparent blur-[1px]"
                        style={{ width: '40%' }}
                    ...
                    />
                    <motion.div 
                        className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-blue-600 rounded-full shadow-[0_0_10px_#2563eb]"
...
// 4. Log Item Component (Inside Phone - Dark Mode preserved for contrast)
...
// 5. Scroll Section Component (Updated for better text visibility)
const ScrollSection = ({ title, description, children, id, isActive, gradient }) => (
    <div 
        id={id}
        className={`scroll-section min-h-screen flex flex-col justify-center px-4 transition-all duration-700 ${isActive ? 'opacity-100 translate-x-0' : 'opacity-30 translate-x-[-20px] blur-[1px]'}`}
    >
        <motion.h2 
        ...
        >
            <span className={`bg-clip-text text-transparent bg-gradient-to-r ${gradient} animate-shine pb-2`}>
                {title}
            </span>
        </motion.h2>
        
        {/* ÖKAD KONTRAST PÅ BESKRIVNINGEN */}
        <p className="text-xl text-slate-800 max-w-xl mb-8 leading-relaxed font-medium">
            {description}
        </p>
...
// Helper for Text Features (Updated for better text visibility)
const FeatureBox = ({ icon: Icon, title, text }) => (
    <div className="flex items-start gap-4 p-5 rounded-2xl border border-slate-200 bg-white hover:border-blue-200 hover:shadow-lg hover:shadow-blue-900/5 transition-all duration-300 group">
        <div className="p-2.5 rounded-xl bg-slate-100 text-slate-600 group-hover:bg-blue-50 group-hover:text-blue-700 transition-colors">
            <Icon size={20} />
        </div>
        <div>
            {/* TYDLIGARE RUBRIK */}
...
const EuroLedgerDemo = () => { 
    ...
    return (
        <div className="relative pt-0 pb-0 bg-white" ref={componentRef}>
            
            {/* SPLIT BACKGROUND LAYOUT */}
            <div className="absolute inset-0 flex pointer-events-none">
                <div className="w-full md:w-1/2 bg-white" />
                <div className="hidden md:block w-1/2 bg-slate-50 border-l border-slate-100" />
            </div>

            <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-start relative">
                
                {/* --- LEFT COLUMN: SCROLLABLE TEXT --- */}
                <div className="relative z-20 w-full md:w-1/2 md:pr-12 lg:pr-20">
                    <ScrollSection 
                        id="step-0" 
                ...
                <div className="hidden md:flex w-1/2 h-screen sticky top-0 items-center justify-center pointer-events-none"> 
                    <div className="relative w-full max-w-lg h-[600px] flex items-center justify-center overflow-visible pointer-events-auto">
                        ...
                        >
                            ...
                            {/* Screen Content (Dark UI for Contrast) */}
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[700px] bg-slate-300/30 blur-[120px] -z-10 rounded-full pointer-events-none mix-blend-multiply"></div>
                    </div>
                </div>
            </div>
        </div>
