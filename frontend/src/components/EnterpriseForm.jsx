...
import { 
  ...
  CheckCircle2, Globe, Zap, Cpu, Lock, AlertCircle, Briefcase, Info, Cookie
} from 'lucide-react';

// --- InputGroup och StepItem components (utgått från att de finns) ---
const InputGroup = ({ label, name, required, disabled, ...props }) => (
  <div className="space-y-1">
    ...
    <input name={name} {...props} className="w-full bg-white border border-slate-200 focus:border-[#635bff] focus:ring-4 focus:ring-[#635bff]/10 rounded-xl p-3 text-sm transition-all outline-none font-medium text-slate-900 disabled:bg-slate-50 disabled:text-slate-400" disabled={disabled} />
  </div>
);
...
// --- Cookie Consent Component (Integrerad i EnterpriseForm) ---
const GDPRCookieConsent = ({ onAccept, onDecline, onReadPolicy }) => {
  return (
    <motion.div 
      ...
      className="absolute inset-0 flex items-center justify-center bg-white/90 backdrop-blur-md p-6 rounded-[2rem] z-30" 
    >
      <div className="bg-slate-50 p-6 rounded-xl shadow-xl border border-slate-200 max-w-sm text-center">
        <Cookie size={32} className="text-blue-600 mx-auto mb-3" />
        <h4 className="text-xl font-bold text-slate-900 mb-2">Privacy Required</h4>
        <p className="text-sm text-slate-600 mb-5">
          To proceed with the Enterprise form, please accept our privacy policy regarding essential data processing.
        </p>
        ...
        <button onClick={() => onDecline('denied')} className="text-xs text-slate-500 mt-4 hover:text-slate-800 underline">Decline (Essential only)</button>
      </div>
    </motion.div>
  );
};

const EnterpriseForm = ({ unifiedCookieStatus, onUnifiedConsent, onOpenPrivacy }) => { // Nyckelfunktion: Tar emot props
  const [step, setStep] = useState(1);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitted, setSubmitted] = useState(false);
  const [isValid, setIsValid] = useState(false);

  // ANVÄNDER GLOBAL STATUS istället för lokal enterprise_cookie_consent
  const formInteractionBlocked = unifiedCookieStatus === null;
  ...
  useEffect(() => {
    let valid = false;
    // Formuläret är endast giltigt om cookies har hanterats (unified status finns)
    if (unifiedCookieStatus === null) {
      setIsValid(false);
      return;
    }
    ...
    setIsValid(valid);
  }, [formData, step, unifiedCookieStatus]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!isValid || unifiedCookieStatus === 'denied') return alert("Form submission requires consent.");
  ...
  // Funktion som anropas av GDPRCookieConsent
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-16 font-sans text-slate-900 relative overflow-hidden">
        {/* INTERAKTIV BAKGRUND: Enkel animerad effekt */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <motion.div 
                className="w-96 h-96 bg-blue-100/50 rounded-full absolute -top-40 -left-40 blur-3xl opacity-50"
                animate={{ scale: [1, 1.1, 1], rotate: [0, 5, 0] }}
                transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
            />
            <motion.div 
                className="w-80 h-80 bg-purple-100/50 rounded-full absolute -bottom-40 -right-40 blur-3xl opacity-50"
                animate={{ scale: [1, 0.9, 1], rotate: [0, -5, 0] }}
                transition={{ duration: 18, repeat: Infinity, ease: "easeInOut", delay: 2 }}
            />
        </div>

      <div className="max-w-5xl mx-auto animate-fade-in-up px-4 md:px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-4 space-y-6 sticky top-24 hidden md:block">
            ...
            <motion.div 
              ...
              className="bg-blue-50 p-5 rounded-2xl border border-blue-100 shadow-md"
            >
          ...
          <div className="md:col-span-8">
            <motion.form 
              className="bg-white rounded-[2rem] shadow-2xl border border-slate-100 overflow-hidden relative min-h-[500px] flex flex-col" 
            ...
            >
              ...
              {/* COOKIE CONSENT - BLOCKERAR FORMULÄRET TILLS ANVÄNDAREN VÄLJER */}
              <AnimatePresence>
                {formInteractionBlocked && (
                    <GDPRCookieConsent 
                        onAccept={onUnifiedConsent} // Använder Unified Handler
                        onDecline={onUnifiedConsent} // Använder Unified Handler
              ...
              <div className="h-1.5 bg-slate-100 w-full md:hidden"><motion.div className="h-full bg-[#635bff]" initial={{ width: 0 }} animate={{ width: `${(step/3)*100}%` }} transition={{ duration: 0.5 }} /></div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 1 && (
                    <motion.div key="step1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }}>
                      ...
                      <p className="text-sm text-slate-500 mb-6">Tell us about your company and your role.</p>
                      <div className="space-y-5 mt-6">
                        <div className="grid md:grid-cols-2 gap-5">
                          <InputGroup label="Full Name" name="name" placeholder="Jane Doe" value={formData.name} onChange={handleChange} required disabled={formInteractionBlocked} />
                          <InputGroup label="Work Email" name="email" type="email" placeholder="jane@corp.com" value={formData.email} onChange={handleChange} required disabled={formInteractionBlocked} />
                        </div>
                        <div className="grid md:grid-cols-2 gap-5">
                          <InputGroup label="Company" name="company" placeholder="Acme Corp" value={formData.company} onChange={handleChange} required disabled={formInteractionBlocked} />
                          <div className="space-y-1">
                            ...
                            <div className="relative">
                              ...
                              <select name="industry" value={formData.industry} onChange={handleChange} className="w-full bg-white border border-slate-200 rounded-xl p-3 pl-10 text-sm font-medium outline-none focus:border-[#635bff] cursor-pointer disabled:bg-slate-50" disabled={formInteractionBlocked}>
                  ...
                  {step === 2 && (
                    <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }}>
                      ...
                      <p className="text-sm text-slate-500 mb-6">Select your primary deployment region and estimated event volume.</p>
                      <div className="space-y-6 mt-6">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="relative">
                              ...
                              <select name="region" value={formData.region} onChange={handleChange} className="w-full bg-white border border-slate-200 rounded-xl p-3 pl-10 text-sm font-medium outline-none focus:border-[#635bff] cursor-pointer appearance-none disabled:bg-slate-50" disabled={formInteractionBlocked}>
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {['< 1M', '1M - 50M', '50M - 500M', '500M+'].map(opt => (
                                <motion.div 
                                    key={opt} 
                                    onClick={() => !formInteractionBlocked && selectVolume(opt)} 
                                    className={`cursor-pointer rounded-xl p-3 border transition-all flex justify-between items-center ${formData.volume === opt ? 'border-[#635bff] bg-[#635bff]/5 ring-1 ring-[#635bff]' : 'border-slate-200 hover:border-slate-300'}`}
                                    whileHover={{ scale: formInteractionBlocked ? 1 : 1.02, transition: { duration: 0.1 } }}
                                    whileTap={{ scale: formInteractionBlocked ? 1 : 0.98 }}
                                    style={{ opacity: formInteractionBlocked ? 0.5 : 1 }}
                                >
                         ...
                         <div className="flex flex-wrap gap-2">
                            {['GDPR', 'CCPA', 'SOC 2', 'HIPAA', 'ISO 27001', 'PCI DSS'].map(tech => (
                              <motion.button 
                                key={tech} 
                                type="button" 
                                onClick={() => !formInteractionBlocked && toggleCompliance(tech)} 
                                className={`px-3 py-2 rounded-lg text-xs font-bold border transition-all flex items-center gap-2 ${formData.compliance.includes(tech) ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-[#635bff]'}`}
                                whileTap={{ scale: formInteractionBlocked ? 1 : 0.95 }}
                                style={{ opacity: formInteractionBlocked ? 0.5 : 1 }}
                                disabled={formInteractionBlocked}
                              >
                      ...
                      <div>
                         ...
                         <div className="grid grid-cols-2 gap-3">
                            {['Immediately', '< 1 Month', '1-3 Months', 'Budgeting'].map(time => (
                               <motion.div 
                                    key={time} 
                                    onClick={() => !formInteractionBlocked && selectTimeline(time)} 
                                    className={`cursor-pointer p-3 rounded-lg border text-xs font-bold text-center transition-all ${formData.timeline === time ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}
                                    whileHover={{ scale: formInteractionBlocked ? 1 : 1.02 }}
                                    whileTap={{ scale: formInteractionBlocked ? 1 : 0.98 }}
                                    style={{ opacity: formInteractionBlocked ? 0.5 : 1 }}
                               >
                                  {time}
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center shrink-0">
                 ...
                 {step < 3 ? (
                   <button type="button" onClick={nextStep} disabled={!isValid || formInteractionBlocked} className={`px-6 py-3 rounded-xl font-bold text-xs md:text-sm flex items-center gap-2 shadow-lg transition-all ${isValid && !formInteractionBlocked ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>Continue <ArrowRight size={16} /></button>
                 ) : (
                   <button type="submit" disabled={isSubmitting || !isValid || formInteractionBlocked || unifiedCookieStatus === 'denied'} className={`bg-[#635bff] text-white px-6 py-3 rounded-xl font-bold text-xs md:text-sm flex items-center gap-2 hover:bg-[#5449e3] shadow-lg disabled:opacity-70 ${!isValid || formInteractionBlocked || unifiedCookieStatus === 'denied' ? 'cursor-not-allowed' : ''}`}>{isSubmitting ? 'Sending...' : <><Send size={16} /> Request Access</>}</button>
                 )}
              </div>
            </motion.form>
          </div>
