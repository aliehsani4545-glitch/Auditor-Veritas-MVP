...
import { 
  ...
  ScrollText, AlertCircle, Server, CreditCard, Ban, X, Code, Fingerprint, Globe 
} from 'lucide-react';
                ...
                <div className="prose prose-sm max-w-none prose-headings:text-slate-900 prose-p:text-slate-600">
                  ...
                  <Section title="1. Data Processor Agreement (DPA)" icon={FileText} color="blue">
                    <p className="text-sm mb-3">
                      By using Auditor Veritas, you designate us as a <strong>Data Processor</strong> under GDPR Article 28. 
                      We process data solely on your documented instructions.
                    </p>
                    <ul className="list-disc pl-5 space-y-1 text-sm marker:text-blue-500">
                      ...
                      <li><strong>Duration:</strong> For the term of your subscription.</li>
                  ...
                  <Section title="2. Data Residency & Security" icon={Server} color="indigo">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <FeatureBox title="Storage Location" desc="AWS EU (Ireland)" verified />
                  ...
                  <Section title="3. PII & Pseudonymization" icon={Fingerprint} color="purple">
                    <p className="text-sm mb-3">
                      To minimize risk, we enforce <strong>Client-Side Pseudonymization</strong> for user identifiers.
                    </p>
                    <ul className="list-disc pl-5 space-y-1 text-sm marker:text-purple-500">
                      <li><strong>User IDs:</strong> Hashed (SHA-256) on your client before transmission. We never see raw IDs.</li>
                      <li><strong>Payloads:</strong> You are responsible for not including clear-text PII in the JSON `metadata` field unless encrypted.</li>
                      <li><strong>Right to Erasure:</strong> We provide API endpoints to delete all logs associated with a specific hash key.</li>
                    </ul>
                  </Section>

                  <Section title="4. Sub-processors" icon={Users} color="slate">
                    <p className="text-sm">
                      We engage the following third-party entities to assist with our services:
                    </p>
                    <div className="mt-3 overflow-hidden rounded-lg border border-slate-200">
                      <table className="w-full text-xs text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          <tr><td className="p-2">Amazon Web Services</td><td className="p-2">Hosting & DB</td><td className="p-2">Ireland (EU)</td></tr>
                          <tr><td className="p-2">Stripe</td><td className="p-2">Payments</td><td className="p-2">USA (DPA in place)</td></tr>
                          <tr><td className="p-2">Netlify</td><td className="p-2">Frontend CDN</td><td className="p-2">Global</td></tr>
                        </tbody>
                      </table>
                    </div>
                  </Section>
              ...
              <motion.div key="terms" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-8 pb-8">
                ...
                <div className="prose prose-sm max-w-none prose-headings:text-slate-900 prose-p:text-slate-600">
                  <Section title="1. Acceptable Use Policy" icon={AlertCircle} color="orange">
                    <p className="text-sm mb-3">
                      You agree to use the API only for lawful purposes. You strictly agree <strong>NOT</strong> to:
                    </p>
                    <ul className="space-y-2 bg-orange-50/50 p-4 rounded-xl border border-orange-100 text-sm">
                      ...
                      <li className="flex gap-2"><Ban size={16} className="text-orange-600 shrink-0"/> Use the service to store illegal content, malware, or hate speech.</li>
                      <li className="flex gap-2"><Ban size={16} className="text-orange-600 shrink-0"/> Attempt to circumvent rate limits or access control mechanisms.</li>
                    </ul>
                  </Section>

                  <Section title="2. Service Level Agreement (SLA)" icon={Activity} color="emerald">
                    <p className="text-sm mb-3">
                      For Enterprise plans, we guarantee <strong>99.9% Monthly Uptime</strong>.
                    </p>
                    <ul className="list-disc pl-5 space-y-1 text-sm marker:text-emerald-500">
                      <li><strong>Maintenance:</strong> Scheduled windows are communicated 48h in advance.</li>
                      <li><strong>Credits:</strong> 10% credit for every 1% below guaranteed uptime.</li>
                      <li><strong>Support:</strong> Priority email support with &lt;4h response time for critical incidents.</li>
                    </ul>
                  </Section>

                  <Section title="3. Limitation of Liability" icon={ShieldCheck} color="slate">
                    <p className="text-sm text-justify">
                      TO THE MAXIMUM EXTENT PERMITTED BY LAW, AUDITOR VERITAS SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL OR PUNITIVE DAMAGES, OR ANY LOSS OF PROFITS OR REVENUES. OUR AGGREGATE LIABILITY SHALL NOT EXCEED THE AMOUNT YOU PAID US IN THE PAST TWELVE (12) MONTHS.
                    </p>
                  </Section>

                  <Section title="4. Termination" icon={Ban} color="red">
                    <p className="text-sm">
                      We may terminate or suspend access immediately, without prior notice, for any breach of these Terms. Upon termination, your right to use the Service will immediately cease, and all data will be permanently deleted after a 30-day grace period.
                    </p>
                  </Section>
                </div>
                
                <div className="text-center pt-6 border-t border-slate-200">
                    <p className="text-xs text-slate-400">Governing Law: Sweden (Stockholm District Court)</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="p-6 border-t border-slate-200 bg-white shrink-0 z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] relative">
            {!canAccept && (
              <div className="absolute -top-12 left-0 w-full flex justify-center pointer-events-none">
                <motion.div 
                    initial={{ y: 0 }} animate={{ y: 5 }} transition={{ repeat: Infinity, repeatType: "reverse", duration: 1 }}
                    className="bg-slate-900/90 backdrop-blur text-white text-[10px] font-bold px-4 py-1.5 rounded-full flex items-center shadow-lg gap-2"
                >
            ...
            <button 
              onClick={onAccept}
              disabled={!canAccept}
              className={`w-full py-4 rounded-xl font-bold text-base transition-all duration-300 flex items-center justify-center gap-2 shadow-lg ${canAccept ? 'bg-[#635bff] text-white hover:bg-[#5449e3] active:scale-98 hover:shadow-indigo-500/25' : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'}`}
            >
...
// Hack för att importera lucide icons dynamiskt i Section (Activity behövs importeras högst upp)
