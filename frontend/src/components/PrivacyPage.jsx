...
const PrivacyPage = ({ onAccept, isFooterView = false, onClose, initialTab = 'privacy' }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[#020617]/90 backdrop-blur-xl flex items-center justify-center sm:p-4">
      <motion.div 
        initial={{ scale: 0.95, opacity: 0 }} 
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.95, opacity: 0 }}
        transition={{ type: "spring", damping: 25, stiffness: 300 }}
        className="w-full sm:max-w-4xl bg-[#0f172a] sm:rounded-3xl rounded-none shadow-2xl border border-white/10 overflow-hidden flex flex-col h-full sm:h-[85vh] relative"
      >
        ...
        {!isFooterView && (
          <div className="p-6 border-t border-white/10 bg-[#0f172a] shrink-0 z-20 relative">
            {!canAccept && (
