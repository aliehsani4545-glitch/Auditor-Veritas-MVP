...
      <div className={`p-2 rounded-lg bg-${color}-500/10 mr-3`}><Icon className={`w-5 h-5 text-${color}-600`} /></div>
...
  // Ljus bakgrund med grå kantlinje
  <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex justify-between items-center">
    <div>
      <p className="font-bold text-slate-700 text-xs">{title}</p>
      <p className="text-[11px] text-slate-500">{desc}</p>
    </div>
    {/* Ikonfärg ändras till en djupare nyans för kontrast */}
    {verified && <div className="text-emerald-600"><CheckCircleMini /></div>}
  </div>
);
...
// --- HUVUDKOMPONENT (UPPDATERAD FÖR LJUS TEMA) ---
...
          <button onClick={onClose} className="absolute top-6 right-6 z-50 p-2 bg-slate-100 hover:bg-slate-200 rounded-full text-slate-800 transition-colors backdrop-blur-md border border-slate-200">
            <X size={20} />
          </button>
        )}

        <div className="p-8 pb-0 text-center shrink-0 relative z-10">
          {/* Ljusa färger för etiketter */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 text-xs font-bold uppercase tracking-wide mb-4 backdrop-blur-md">
             <Globe size={12} /> Legal Center
          </div>
          {/* Textfärger är nu mörka */}
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight mb-2">Compliance & Terms</h1>
          <p className="text-slate-500 text-sm mb-8 max-w-md mx-auto">
            Transparent policies designed for enterprise security and GDPR compliance.
          </p>

          <div className="flex p-1 bg-slate-100 rounded-xl mb-8 relative border border-slate-200 backdrop-blur-xl max-w-md mx-auto">
            {/* Ljusa flikar: texten blir mörk när inaktiv */}
            <button onClick={() => setActiveTab('privacy')} className={`flex-1 py-3 text-xs sm:text-sm font-bold rounded-lg transition-all relative z-10 flex items-center justify-center gap-2 ${activeTab === 'privacy' ? 'text-white' : 'text-slate-600 hover:text-slate-900'}`}>
              <Lock size={14} /> Privacy Policy
            </button>
            <button onClick={() => setActiveTab('terms')} className={`flex-1 py-3 text-xs sm:text-sm font-bold rounded-lg transition-all relative z-10 flex items-center justify-center gap-2 ${activeTab === 'terms' ? 'text-white' : 'text-slate-600 hover:text-slate-900'}`}>
...
                 <p className="text-xs uppercase tracking-wider font-bold text-emerald-600 mb-4">Effective Date: November 24, 2025</p>
                 
...
                   </p>
                   <ul className="list-disc pl-5 space-y-1 text-sm marker:text-emerald-600 text-slate-600">
...
                   <p className="text-sm mb-3 text-slate-700">By using Auditor Veritas, you designate us as a <strong>Data Processor</strong> under GDPR Article 28.</p>
                   <button onClick={downloadDPA} className="inline-flex items-center gap-2 text-xs font-bold text-blue-700 bg-blue-500/10 px-4 py-2 rounded-lg hover:bg-blue-500/20 transition-colors border border-blue-500/30">
...
                   <p className="text-sm mb-3 text-slate-700">You agree to use the API lawfully. You strictly agree <strong>NOT</strong> to:</p>
                   {/* Ljusare, mer kontrasterande bakgrund för varningslistan */}
                   <ul className="space-y-2 bg-red-500/10 p-4 rounded-xl border border-red-500/30 text-sm text-red-700">
                     <li className="flex gap-2"><Ban size={16} className="shrink-0 text-red-500"/> Reverse engineer the cryptographic proof engine.</li>
                     <li className="flex gap-2"><Ban size={16} className="shrink-0 text-red-500"/> Store illegal content or malware.</li>
                     <li className="flex gap-2"><Ban size={16} className="shrink-0 text-red-500"/> Circumvent rate limits.</li>
                   </ul>
                 </Section>
...
                   <p className="text-sm mb-3 text-slate-700">Enterprise plans guarantee <strong>99.9% Uptime</strong>.</p>
                   <ul className="list-disc pl-5 space-y-1 text-sm marker:text-blue-600 text-slate-600">
...
                   <p className="text-sm text-slate-500 text-justify">TO THE MAXIMUM EXTENT PERMITTED BY LAW, AUDITOR VERITAS SHALL NOT BE LIABLE FOR INDIRECT DAMAGES. AGGREGATE LIABILITY SHALL NOT EXCEED THE AMOUNT PAID IN THE PAST 12 MONTHS.</p>
                 </Section>
              </motion.div>
            )}
          </AnimatePresence>
...
        {/* Bottensektionen (Accept-knappen) */}
        {!isFooterView && (
          {/* Ljus bakgrund i botten */}
          <div className="p-6 border-t border-slate-200 bg-white shrink-0 z-20 relative">
            {!canAccept && (
...
              </div>
            )}
            {/* Knappen har nu en ljusare inaktiv stil */}
            <button onClick={onAccept} disabled={!canAccept} className={`w-full py-4 rounded-xl font-bold text-base transition-all duration-300 flex items-center justify-center gap-2 shadow-lg ${canAccept ? 'bg-blue-600 text-white hover:bg-blue-500 hover:shadow-blue-500/25' : 'bg-slate-200 text-slate-500 cursor-not-allowed border border-slate-300'}`}>
